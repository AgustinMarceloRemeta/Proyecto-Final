...
{    
    ...
    private void GerarDias()
    {
        ...
        for (int i = (int)primeiroDia.DayOfWeek, dia = 1; dia <= DateTime.DaysInMonth(_dataCalendarioExibido.Year, _dataCalendarioExibido.Month); i++, dia++)
        {
            ...
            int diaDaSemana = (i % 7); // Para garantir que não ultrapasse o índice 6
            if (diaDaSemana == 0 || diaDaSemana == 6) // 0 = domingo, 6 = sábado
            {
                // Aquí cambiamos el booleano que indica fin de semana
                _dias[i].weekend=true;
            }
            else
            {
                _dias[i].weekend = false;
            }
            _dias[i].SetButton();
        }
    }
